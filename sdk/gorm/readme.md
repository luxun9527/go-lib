...
1、mkdir -p /root/docker/mysql/conf && mkdir -p /root/docker/mysql/data
2、将my.cnf放到 /root/docker/mysql/conf 下
3、docker 创建容器
docker run -p 33606:3306 --name mysql \
...
参考 https://gorm.io/zh_CN/docs/data_types.html
...
gen 文档，gorm虽然已经是一个比较方便的orm，但是在where条件，指定查询字段的时候需要我们手动指定列名，gen则给我们提供了很多方便，避免了手动指定列名，此外gen还根据表名生成对应的结构体，墙裂推荐使用gen。

### 读写分离

https://www.cnblogs.com/aozhejin/p/15847969.html
